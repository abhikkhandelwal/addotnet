...
public class WebUIAutomation extends TestBase {
	...
	public static boolean checkTestCaseRunmode(String testCase){
		...
		for(int i=2; i<=datatable.getRowCount("AD.NET"); i++){
			
			if(datatable.getCellData("AD.NET", "Test Case ID", i).equalsIgnoreCase(testCase)){
				
				if(datatable.getCellData("AD.NET", "Runmode", i).equalsIgnoreCase("Y")){
